...
return ['$scope', '$location', 'mnemonics', 'tx_sender', 'notices', 'wallets', '$window', 'facebook', '$modal', 'gaEvent', '$q', 'reddit', 'storage', 'trezor', 'btchip', 'bip38', '$interval', '$sce', 'hw_detector',
        function SignupController($scope, $location, mnemonics, tx_sender, notices, wallets, $window, facebook, $modal, gaEvent, $q, reddit, storage, trezor, btchip, bip38, $interval, $sce, hw_detector) {
    ...
    var signup_with_trezor = function(hd_deferred) {
        trezor.getDevice().then(function(trezor_dev) {
            trezor_dev.getPublicKey([]).then(function(result) {
                var hdwallet = Bitcoin.HDNode.fromBase58(result.message.xpub);
                hd_deferred.resolve({
                    master_public: hdwallet.pub.toHex(),
                    master_chaincode: BitcoinAux.bytesToHex(hdwallet.chaincode),
...
//            entropy = Bitcoin.getBigRandom(TWOPOWER256).toByteArrayUnsigned();
	    //entropy = Bitcoin.BigInteger(TWOPOWER256.bitLength(), Bitcoin.rng);
	    //entropy = entropy.mod(TWOPOWER256.subtract(Bitcoin.BigInteger.ONE));
	    //entropy = entropy.add(Bitcoin.BigInteger.ONE);
	    entropy = Bitcoin.rng(32); //Byte array of size 32
            $scope.signup.seed = BitcoinAux.bytesToHex(entropy);
            ...
            mnemonics.toMnemonic(entropy).then(function(mnemonic) {
                mnemonics.toSeed(mnemonic).then(function(seed) {
                    mnemonics.toSeed(mnemonic, 'greenaddress_path').then(function(path_seed) {
                        $q.when(Bitcoin.HDNode.fromSeedHex(seed, cur_net)).then(function(hdwallet) {
                            secured_confirmed.promise.then(function() {
                                ...
                                hd_promise.then(function(hd) {
                                    tx_sender.call('http://greenaddressit.com/login/register',
                                            hd.master_public, hd.master_chaincode).then(function(data) {
                                        if (hd.btchip_pubkey) {
    ...
    $scope.signup.encrypt_mnemonic = function() {
        ...
        bip38.encrypt_mnemonic_modal($scope, BitcoinAux.hexToBytes($scope.signup.seed)).then(function(encrypted) {
