...
t=r.binLen,l=r.value):n("inputFormat must be HEX, TEXT, ASCII, or B64");a=8*j;b=j/4-1;j<t/8?(l=$(l,t,d),l[b]&=4294967040):j>t/8&&(l[b]&=4294967040);for(j=0;j<=b;j+=1)y[j]=l[j]^909522486,z[j]=l[j]^1549556828;d=$(z.concat($(y.concat(k),a+g,d)),a+p,d);return h(d,G(f))}};
    ...
    function PRF(password, salt) {
        var shaObj = new jsSHA(salt, "HEX");
        return shaObj.getHMAC(password, "HEX", "SHA-512", "HEX");
    }
    ...
    while (derivedKeyHex.length < keylen*2) {
        var cur_salt = salt.concat(BitcoinAux.bytesToHex(BitcoinAux.wordsToBytes([blockindex])));
        var block = PRF(password, cur_salt);
        for (var u = block, i = 1; i < iterations; i++) {
            u = PRF(password, u);
            block_bytes = BitcoinAux.hexToBytes(block);
            u_bytes = BitcoinAux.hexToBytes(u);
            for (var j = 0; j < block_bytes.length; j++) block_bytes[j] ^= u_bytes[j];
            block = BitcoinAux.bytesToHex(block_bytes);
        ...
        derivedKeyHex = derivedKeyHex.concat(block);
        blockindex++;
    }
